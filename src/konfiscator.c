...
void *malloc(size_t size)
{
	if (list_empty(&free_list)) {
		void *addr = mmap((void *)0, FIRST_BLOCK_SIZE,
...
void free(void *ptr)
{
	...
	// we take the pointer and use container_of to get the corresponding
	// block
...
void increment_metric(int metric_index)
{
	...
	counts = mmap(0, sizeof(*counts) * NUM_COUNTERS, PROT_WRITE, MAP_SHARED,
		      fd, 0);
